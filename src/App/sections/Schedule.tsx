...
const Schedule = (
    ...
    const handleWheel = (e: any) => {
        ...
        if (courseListElements.some((_element) => {
            return _element.contains(e.target) && _element.scrollHeight > _element.clientHeight
        })) { return }
    ...
    const onDropWithTerm = (dropResult: DropResult, termName: string) => {
        ...
        if (dropResult.removedIndex !== null) {
            if (termName === "shortlist") {
                removeShortList(dropResult.payload)
            } else {
                removeCourseFromList({termName, index: dropResult.removedIndex})
            }
        }
        if (dropResult.addedIndex !== null) {
            if (termName === "shortlist") {
                addShortList({code: dropResult.payload, index: dropResult.addedIndex})
            } else {
                addCourseToList({termName, index: dropResult.addedIndex, code: dropResult.payload})
            }
        }
